...
//    [backButton setTitle:@"Back" forState:UIControlStateNormal];
    ...
    if ([[[self.photo objectForKey:kPAPPhotoUserKey] objectId] isEqualToString:[[PFUser currentUser] objectId]]) {
        self.navigationItem.rightBarButtonItem = [[UIBarButtonItem alloc] initWithBarButtonSystemItem:UIBarButtonSystemItemTrash target:self action:@selector(actionButtonAction:)];
...
- (BOOL)textFieldShouldReturn:(UITextField *)textField {
    ...
    if (trimmedComment.length != 0 && [self.photo objectForKey:kPAPPhotoUserKey]) {
        ...
        [comment saveEventually:^(BOOL succeeded, NSError *error) {
            ...
            [[NSNotificationCenter defaultCenter] postNotificationName:PAPPhotoDetailsViewControllerUserCommentedOnPhotoNotification object:self.photo userInfo:[NSDictionary dictionaryWithObject:[NSNumber numberWithInt:(int)self.objects.count + 1] forKey:@"comments"]];
...
-(void)actionSheet:(UIActionSheet *)actionSheet clickedButtonAtIndex:(NSInteger)buttonIndex {
    if (actionSheet.tag == MainActionSheetTag) {
        if ([actionSheet destructiveButtonIndex] == buttonIndex) {
            UIActionSheet *actionSheet = [[UIActionSheet alloc] initWithTitle:@"本当にこの投稿を削除しますか？" delegate:self cancelButtonTitle:@"キャンセル" destructiveButtonTitle:@"はい、削除します" otherButtonTitles:nil];
...
- (void)actionButtonAction:(id)sender {
    UIActionSheet *actionSheet = [[UIActionSheet alloc] initWithTitle:nil delegate:self cancelButtonTitle:@"キャンセル" destructiveButtonTitle:@"投稿を削除" otherButtonTitles:nil];
